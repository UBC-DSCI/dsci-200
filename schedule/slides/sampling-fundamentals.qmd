...
*This material is adapted from* [*Chapter 5 of Elementary Statistics with R*](https://homerhanumat.github.io/elemStats/).
...
- From our population distribution of temperature, we may want to compute some quantities (e.g., mean, median, standard deviation).
- A **parameter** is a numerical quantity that describes the **population**.
- Here are some parameters computed from our population of wildfire temperatures:
...
- A <span class="secondary">sample</span> is a subset of our population that we will use to draw conclusions about the larger population. 
...
- A **statistic** is a numerical quantity that describes the **sample**.
  - When we use a statistic to estimate a population parameter, it is often referred to as a *point estimate*. 
- Here are some statistics computed from our sample of wildfire temperatures:
...
- B) The new sample mean will likely be different, but close to the first one  
...
ggplot(combined_samples, aes(x = temperature, fill = sample)) +
  ...
  labs(
    title = "Overlaied Histograms of Two Wildfire Samples",
    x = "Temperature (°C)",
    y = "Count",
    fill = "Sample") +
  theme_minimal()
...
- The variation in point estimates across samples is called **sampling variability**.
...
Let’s use this [sampling distribution app](https://www.zoology.ubc.ca/~whitlock/Kingfisher/SamplingNormal.htm), by Dr. Whitlock (UBC), to understand what the sampling distribution is.
...
- A) We all got the population mean equal to 106

- B) We all got the same sample mean close to 106

- C) We all got different sample means close to 106
...
- A) The distribution of the sample means is similar to the sample distribution of fish lengths

- B) We all get the same distribution of sample means

- C) We all get different distributions, but look like the sample distribution of fish lengths

- D) We all get different distributions, but look bell-shaped
...
ggplot(combined_sampling, aes(x = sample_mean, fill = sample_size)) +
  ...
  labs(
    title = "Overlaied Sampling Distributions of the Sample Mean",
    x = "Sample Mean Temperature (°C)",
    y = "Count",
    fill = "Sample Size"
  ) +
