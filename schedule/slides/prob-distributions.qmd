...
- The odds of winning the Powerball lottery with a single ticket are about 1 in 300 million.
:::
...
::: {.callout-note}
In many situations, we won't be focused on defining the set of all possible outcomes (aka sample space). However, it can help us understand what is plausible
:::
...
## A different example of a random phenomenon
...
- **Random variable**:  a numerical quantity defined from the outcome of a random phenomenon (e.g., number of heads when tossing 2 coins, temperature the day of a fire)
...
- Some distributions have special names (e.g., Bernoulli, Normal)
:::
...
- Random variables are classified based on the type of values they can take:	
	...
	- Continuous: values fall in a range and can take infinitely many possible values
- Examples:
...
- $\lambda$ is the rate parameter (average number of events per unit time)  
...
## iClicker 2 
<br>
Your data science team is analyzing the number of times customers log into an app during a single day. Login events happen independently, and you want to model how many logins there are per user per day.
