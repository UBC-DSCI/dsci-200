...
- Use simulations to approximate probabilities of events or distribution functions.
...
Which of these random processes can we easily repeat thousands of times? Which are difficult or impossible to reproduce in practice?
...
- approximate a probability that is hard to compute analytically
...
- repeat 15 times
- use [**this link**](https://app.sli.do/event/81ZjecCAZNYZUdvhR3TUQ1) to share what you found
...
## iClicker 1: define the random process
...
Every time we run this code, we’re simulating one roll of three fair dice! What do we record from each run??
...
You played with the random generator app and already made a decision about what to record. **What did you record from each run?**
...
> We approximate each probability by the proportion of runs in which each sum occurs.
...
#### To make a simulation reproducible, we set the seed to any value!
