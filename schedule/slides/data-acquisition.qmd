...
> *Note: 	The `as` argument is the desired type of output: raw, text or parsed. `content` attempts to automatically figure out which one is most appropriate, based on the content-type. In this example, we will show `raw` first to illustrate what is happening under the hood, but in practice you will often want to use text or parsed for working with JSON or text-based API responses.*
...
Now, we can achieve the same result using the argument `as = "text"`: 
...
Or alternatively (since the raw data is in a simple JSON file format already):

```{r}
data <- fromJSON("https://gbfs.citibikenyc.com/gbfs/en/station_information.json")
stations <- data$data$stations
head(stations)
```
